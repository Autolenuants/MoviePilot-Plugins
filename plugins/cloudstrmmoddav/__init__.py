                ...
                if not self._alist_webdav:
                    ...
                    for source_file in self._webdav_list_files(source_dir, dav_user, dav_pass):
                        ...
                        if (source_file.find("/@Recycle") != -1
                            ...
                            continue
                        # 不复制非媒体文件时直接过滤掉非媒体文件
                        if not self._copy_files and not source_file.lower().endswith(self._video_formats) or file.lower().endswith("mka"):
                            continue
                        if source_file not in self.__cloud_files:
                            logger.info(f"扫描到新文件 {source_file}，正在开始处理")
                            # 云盘文件json新增
            ...
            if not self._alist_webdav:
                ...
                for source_file in self._webdav_list_files(source_dir, dav_user, dav_pass):
                    ...
                    if (source_file.find("/@Recycle") != -1
                        ...
                        continue
                    # 不复制非媒体文件时直接过滤掉非媒体文件
                    if not self._copy_files and not source_file.lower().endswith(self._video_formats) or source_file.lower().endswith("mka"):
                        continue
                    if source_file not in self.__cloud_files:
                        logger.info(f"扫描到新文件 {source_file}，正在开始处理")
                        # 云盘文件json新增
